...
class KLineModel: NSObject {
    ...
    class func cacheKLineModels(klineType: KLineType, goodType: String) {
        let _ = autoreleasepool(invoking: {
            queue.async(execute: {
                let realm = try! Realm()
                var classType = KChartModel.self
                switch klineType{
                    case .miu15:
                        classType = KChartModelQuarter.self
                        break
                    case .miu60:
                        classType = KChartModelHour.self
                    break
                    default:
                        break
                }
                ...
                let queryStr = NSPredicate.init(format: "goodType = %@",goodType)
                var goodMaxTime: Int = 0
                if let maxTimeGood = realm.objects(classType).sorted(byProperty: "priceTime").filter(queryStr).last{
                    goodMaxTime = maxTimeGood.priceTime
                }
//                var goodMinTime: Int = 0
    ...
    class func query15KLineModels(fromTime: Int, toTime: Int, goodType: String, complete: @escaping CompleteBlock){
        let _ = autoreleasepool(invoking: {
            queue.async(execute: {
                ...
                let realm = try! Realm()
                let queryStr = NSPredicate.init(format: "goodType = %@",goodType)
                let result = realm.objects(KChartModelQuarter.self).sorted(byProperty: "priceTime").filter(queryStr).filter("priceTime > \(fromTime)").filter("priceTime < \(toTime)")
                for model in result {
                    models.append(model)
                }
                complete(models as AnyObject?)
